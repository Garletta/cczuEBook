...
public class FlipView extends View {
    ...
    private void drawCurrentAreaAndShadow(Canvas canvas) {
        ...
        if (mFlipMode == FlipMode.RIGHT_BOTTOM) {
            ...
            double degree = Math.PI / 4 + Math.atan2(mTouch.x - mBezierControl1.x, mBezierControl1.y - mTouch.y);
            float rotateDegrees = (float) Math.toDegrees(Math.atan2(mTouch.x - mBezierControl1.x, mBezierControl1.y - mTouch.y));
            ...
            mCurShadowRL.setBounds(left, (int) (mBezierControl1.y - mDiagonalLength), right, (int) (mBezierControl1.y));
            ...
            canvas.clipPath(mFoldAndNextPath, Region.Op.DIFFERENCE);
            canvas.clipPath(tempPath);
            mCurShadowBT.setBounds((int) (mTouch.x - 2f*width), topY, (int) (mTouch.x + mDiagonalLength), btmY);
            ...
            //折叠三角形底部直角边阴影
            Path tempPath = new Path();
            double degree = Math.PI / 4 + Math.atan2(mBezierControl1.x - mTouch.x, mTouch.y - mBezierControl1.y);
            float rotateDegrees = (float) Math.toDegrees(Math.atan2(mBezierControl1.x - mTouch.x, mTouch.y - mBezierControl1.y));
            ...
            canvas.clipPath(mFoldAndNextPath, Region.Op.DIFFERENCE);
            canvas.clipPath(tempPath);
            mCurShadowRL.setBounds(left, (int) mBezierControl1.y, right, (int) (mBezierControl1.y + mDiagonalLength));
            canvas.rotate(rotateDegrees, mBezierControl1.x, mBezierControl1.y);
            mCurShadowRL.draw(canvas);
            canvas.restore();
            //还原坐标系，绘制折叠三角形左侧直角边阴影
            ...
            canvas.clipPath(mFoldAndNextPath, Region.Op.DIFFERENCE);
            canvas.clipPath(tempPath);
            mCurShadowTB.setBounds((int) (mTouch.x - 2f*width), topY, (int) (mTouch.x + mDiagonalLength), btmY);
            canvas.rotate(rotateDegrees, mTouch.x, mTouch.y);
    ...
    private void drawFoldAreaAndShadow(Canvas canvas) {
        if (mFlipMode == FlipMode.RIGHT_BOTTOM) {
            ...
            float rotate = (float) Math.toDegrees(Math.PI / 2f - Math.atan2(mTouch.y - mBezierControl2.y, mViewWidth - mTouch.x));
            canvas.save();
            ...
            float rotateDegrees = (float) Math.toDegrees(Math.PI / 2f + Math.atan2(mViewHeight - mBezierStart2.y, mViewWidth - mBezierStart1.x));
            //还原坐标轴，绘制阴影
            canvas.save();
            canvas.clipPath(mFoldAndNextPath);
            canvas.clipPath(mFoldPath, Region.Op.INTERSECT);
            canvas.rotate(-rotateDegrees, mBezierStart1.x, mBezierStart1.y);
            mFoldShadowRL.setBounds(left, (int) mBezierStart1.y, right, (int) (mBezierStart1.y + mDiagonalLength));
            mFoldShadowRL.draw(canvas);
            canvas.restore();
        } else if (mFlipMode == FlipMode.RIGHT_TOP) {
            //计算填充折叠区域时需要的画布坐标旋转角度
            float rotate = (float) Math.toDegrees(Math.PI / 2f - Math.atan2(mBezierControl2.y - mTouch.y, mBezierControl2.x - mTouch.x));
            canvas.save();
            canvas.clipPath(mFoldAndNextPath);
            canvas.clipPath(mFoldPath, Region.Op.INTERSECT);
            canvas.scale(-1, 1);//x方向镜像
            ...
            int left = (int) (mBezierStart1.x - width - 1);
            int right = (int) (mBezierStart1.x + 1);
            float rotateDegrees = (float) Math.toDegrees(Math.PI / 2f + Math.atan2(mBezierStart2.y, mViewWidth - mBezierStart1.x));
            //还原坐标轴，绘制阴影
            canvas.save();
            canvas.clipPath(mFoldAndNextPath);
            canvas.clipPath(mFoldPath, Region.Op.INTERSECT);
            canvas.rotate(rotateDegrees, mBezierStart1.x, mBezierStart1.y);
            mFoldShadowRL.setBounds(left, (int) (mBezierStart1.y - mDiagonalLength), right, (int) mBezierStart1.y);
            mFoldShadowRL.draw(canvas);
            canvas.restore();
        }
    }
    ...
    private void drawNextAreaAndShadow(Canvas canvas) {
        ...
        if (mFlipMode == FlipMode.RIGHT_BOTTOM) {
            float rotateDegrees = (float) Math.toDegrees(Math.PI / 2f + Math.atan2(mCorner.y - mBezierStart2.y, mCorner.x - mBezierStart1.x));
            canvas.rotate(-rotateDegrees, mBezierStart1.x, mBezierStart1.y);
            mNextShadowRL.setBounds(left, (int) mBezierStart1.y, right, (int) (mDiagonalLength + mBezierStart1.y));
        } else if (mFlipMode == FlipMode.RIGHT_TOP) {
            float rotateDegrees = (float) Math.toDegrees(Math.PI / 2f + Math.atan2(mCorner.y + mBezierStart2.y, mCorner.x - mBezierStart1.x));
            canvas.rotate(rotateDegrees, mBezierStart1.x, mBezierStart1.y);
            mNextShadowRL.setBounds(left, (int) (mBezierStart1.y - mDiagonalLength), right, (int) mBezierStart1.y);
        }
    ...
    private void initObjects() {
        ...
        mCurShadowRL = new GradientDrawable(GradientDrawable.Orientation.RIGHT_LEFT, frontShadowColors);
        mCurShadowRL.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        mCurShadowBT = new GradientDrawable(GradientDrawable.Orientation.BOTTOM_TOP, frontShadowColors);
        mCurShadowBT.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        mCurShadowTB = new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM, frontShadowColors);
        mCurShadowTB.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        int[] color = {0x00333333, 0xb0333333}; //从浅到深
        mFoldShadowRL = new GradientDrawable(GradientDrawable.Orientation.RIGHT_LEFT, color);
        mFoldShadowRL.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        mFoldShadowLR = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, color);
        mFoldShadowLR.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        int[] nextShadowColors = new int[]{0xff111111, 0x00111111}; //从深到浅
        mNextShadowRL = new GradientDrawable(GradientDrawable.Orientation.RIGHT_LEFT, nextShadowColors);
        mNextShadowRL.setGradientType(GradientDrawable.LINEAR_GRADIENT);
        mNextShadowLR = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, nextShadowColors);
